...
namespace ProcessChannel {
	...
	bool ProcessOneElectronTwoGammaInternal( DataSet *d ){
	    ...
	    histo_collection -> Add( new TH1D( TString::Format("%s_h_vertexSector"          , d->GetName()) , "; Sector Number; No.Events", 100, 18, 19							         ) );                        
	    histo_collection -> Add( new TH2D( TString::Format("%s_h_vtx_z_vs_sect"         , d->GetName()) , "; Sector Number; Z_{vertex} / cm", 500, 18, 19, 520, -130, 130       	 ) );
	    histo_collection -> Add( new TH1D( TString::Format("%s_trackSign"               , d->GetName()) , "; Sign; No.Events", 5, -2.5, 2.5                                          ) );
	    histo_collection -> Add( new TH1D( TString::Format("%s_minGammaEnergy"          , d->GetName()) , "; Min E(#gamma) / MeV; No.Events/0.1 MeV", 50, 0, 5                       ) );
	    histo_collection -> Add( new TH1D( TString::Format("%s_maxGammaEnergy"          , d->GetName()) , "; Max E(#gamma) / MeV; No.Events/0.1 MeV", 50, 0, 5                       ) );
	    histo_collection -> Add( new TH1D( TString::Format("%s_minE_cosTheta"           , d->GetName()) , "; Cos(#Theta) e - #gamma_{min}; No.Events", 12, -1, 1                     ) );
	    histo_collection -> Add( new TH1D( TString::Format("%s_minE_probInt"            , d->GetName()) , "; Int. Prob. e - #gamma_{min}; No.Events",  12, 0, 1                      ) );
	    histo_collection -> Add( new TH1D( TString::Format("%s_minE_probExt"            , d->GetName()) , "; Ext. Prob. e - #gamma_{min}; No.Events",  12, 0, 1                      ) );
	    histo_collection -> Add( new TH1D( TString::Format("%s_maxE_cosTheta"           , d->GetName()) , "; Cos(#Theta) e - #gamma_{max} ;No.Events", 12, -1, 1                     ) );
	    histo_collection -> Add( new TH1D( TString::Format("%s_maxE_probInt"            , d->GetName()) , "; Int. Prob. e - #gamma_{max}; No.Events",  12, 0, 1                      ) );
	    histo_collection -> Add( new TH1D( TString::Format("%s_maxE_probExt"            , d->GetName()) , "; Ext. Prob. e - #gamma_{max}; No.Events",  12, 0, 1                      ) );
	    histo_collection -> Add( new TH2D( TString::Format("%s_energy_corr"             , d->GetName()) , "; Min E_#gamma + Max E_#gamma / MeV - E_e / Mev",  40, 0, 4, 20, 0, 2     ) );
	    histo_collection -> Add( new TH1D( TString::Format("%s_energy_score_fun"        , d->GetName()) , "; Score function; No. Events",  50, -5, 5                                 ) );
	    histo_collection -> Add( new TH2D( TString::Format("%s_energy_score_vs_tot"     , d->GetName()) , "; Score function; No. Events",  50, 0, 5, 50, -5, 5                       ) );
	    histo_collection -> Add( new TH1D( TString::Format("%s_energy_before"           , d->GetName()) , "Before Cut on Correlation; Tot Energy / MeV; No.Events / 0.1",  50, 0, 50 ) );
	    histo_collection -> Add( new TH1D( TString::Format("%s_energy_after"            , d->GetName()) , "After Cut on Correlation; Tot Energy / MeV; No.Events / 0.1",  50, 0, 5   ) );
	    histo_collection -> Add( new TH2D( TString::Format("%s_h_layer_vs_side"         , d->GetName()) , "Layer vs Side; Side; Layer", 4, -1.5, 2.5, 10, -0.5, 9.5				     ) );
