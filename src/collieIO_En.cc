...
int main(int argc, char* argv[]) {
    ...
    /* 25 */ double PMT_Bi214_weight      = 28.1459           ; TH1D *  PMT_Bi214       = 0; int PMT_Bi214_i      ; if( PMT_Bi214_weight      != 0 ) { fdir = (TDirectory *) infile->Get("PMT_Bi214"      ); fdir->cd(); PMT_Bi214        = (TH1D *) fdir->Get("PMT_Bi214_h_tot_e_energy"      ) -> Clone( "PMT_Bi214"      ) ; PMT_Bi214      -> Sumw2() ; PMT_Bi214      -> Scale( PMT_Bi214_weight      / PMT_Bi214       ->Integral() ) ; bkgdNames.push_back("PMT_Bi214"     ); vbkgd.push_back( PMT_Bi214      ); PMT_Bi214_i      = index; index++; };
    /* 26 */ double PMT_Tl208_weight      = 23.2553           ; TH1D *  PMT_Tl208       = 0; int PMT_Tl208_i      ; if( PMT_Tl208_weight      != 0 ) { fdir = (TDirectory *) infile->Get("PMT_Tl208"      ); fdir->cd(); PMT_Tl208        = (TH1D *) fdir->Get("PMT_Tl208_h_tot_e_energy"      ) -> Clone( "PMT_Tl208"      ) ; PMT_Tl208      -> Sumw2() ; PMT_Tl208      -> Scale( PMT_Tl208_weight      / PMT_Tl208       ->Integral() ) ; bkgdNames.push_back("PMT_Tl208"     ); vbkgd.push_back( PMT_Tl208      ); PMT_Tl208_i      = index; index++; };
    /* 27 */ double PMT_Ac228_weight      = 3.60714           ; TH1D *  PMT_Ac228       = 0; int PMT_Ac228_i      ; if( PMT_Ac228_weight      != 0 ) { fdir = (TDirectory *) infile->Get("PMT_Ac228"      ); fdir->cd(); PMT_Ac228        = (TH1D *) fdir->Get("PMT_Ac228_h_tot_e_energy"      ) -> Clone( "PMT_Ac228"      ) ; PMT_Ac228      -> Sumw2() ; PMT_Ac228      -> Scale( PMT_Ac228_weight      / PMT_Ac228       ->Integral() ) ; bkgdNames.push_back("PMT_Ac228"     ); vbkgd.push_back( PMT_Ac228      ); PMT_Ac228_i      = index; index++; };
    /* 28 */ double PMT_K40_weight        = 16.813            ; TH1D *  PMT_K40         = 0; int PMT_K40_i        ; if( PMT_K40_weight        != 0 ) { fdir = (TDirectory *) infile->Get("PMT_K40"        ); fdir->cd(); PMT_K40          = (TH1D *) fdir->Get("PMT_K40_h_tot_e_energy"        ) -> Clone( "PMT_K40"        ) ; PMT_K40        -> Sumw2() ; PMT_K40        -> Scale( PMT_K40_weight        / PMT_K40         ->Integral() ) ; bkgdNames.push_back("PMT_K40"       ); vbkgd.push_back( PMT_K40        ); PMT_K40_i        = index; index++; };
    /* 29 */ double ScintInn_K40_weight   = 0.333992          ; TH1D *  ScintInn_K40    = 0; int ScintInn_K40_i   ; if( ScintInn_K40_weight   != 0 ) { fdir = (TDirectory *) infile->Get("ScintInn_K40"   ); fdir->cd(); ScintInn_K40     = (TH1D *) fdir->Get("ScintInn_K40_h_tot_e_energy"   ) -> Clone( "ScintInn_K40"   ) ; ScintInn_K40   -> Sumw2() ; ScintInn_K40   -> Scale( ScintInn_K40_weight   / ScintInn_K40    ->Integral() ) ; bkgdNames.push_back("ScintInn_K40"  ); vbkgd.push_back( ScintInn_K40   ); ScintInn_K40_i   = index; index++; };
    /* 30 */ double ScintOut_K40_weight   = 0.601186          ; TH1D *  ScintOut_K40    = 0; int ScintOut_K40_i   ; if( ScintOut_K40_weight   != 0 ) { fdir = (TDirectory *) infile->Get("ScintOut_K40"   ); fdir->cd(); ScintOut_K40     = (TH1D *) fdir->Get("ScintOut_K40_h_tot_e_energy"   ) -> Clone( "ScintOut_K40"   ) ; ScintOut_K40   -> Sumw2() ; ScintOut_K40   -> Scale( ScintOut_K40_weight   / ScintOut_K40    ->Integral() ) ; bkgdNames.push_back("ScintOut_K40"  ); vbkgd.push_back( ScintOut_K40   ); ScintOut_K40_i   = index; index++; };
    /* 31 */ double ScintPet_K40_weight   = 1.00196           ; TH1D *  ScintPet_K40    = 0; int ScintPet_K40_i   ; if( ScintPet_K40_weight   != 0 ) { fdir = (TDirectory *) infile->Get("ScintPet_K40"   ); fdir->cd(); ScintPet_K40     = (TH1D *) fdir->Get("ScintPet_K40_h_tot_e_energy"   ) -> Clone( "ScintPet_K40"   ) ; ScintPet_K40   -> Sumw2() ; ScintPet_K40   -> Scale( ScintPet_K40_weight   / ScintPet_K40    ->Integral() ) ; bkgdNames.push_back("ScintPet_K40"  ); vbkgd.push_back( ScintPet_K40   ); ScintPet_K40_i   = index; index++; };
    /* 32 */ double MuMetal_Pa234m_weight = 0.738993          ; TH1D *  MuMetal_Pa234m  = 0; int MuMetal_Pa234m_i ; if( MuMetal_Pa234m_weight != 0 ) { fdir = (TDirectory *) infile->Get("MuMetal_Pa234m" ); fdir->cd(); MuMetal_Pa234m   = (TH1D *) fdir->Get("MuMetal_Pa234m_h_tot_e_energy" ) -> Clone( "MuMetal_Pa234m" ) ; MuMetal_Pa234m -> Sumw2() ; MuMetal_Pa234m -> Scale( MuMetal_Pa234m_weight / MuMetal_Pa234m  ->Integral() ) ; bkgdNames.push_back("MuMetal_Pa234m"); vbkgd.push_back( MuMetal_Pa234m ); MuMetal_Pa234m_i = index; index++; };
    /* 33 */ double Cd116_2b2n_m14_weight = 5020.49           ; TH1D *  Cd116_2b2n_m14  = 0; int Cd116_2b2n_m14_i ; if( Cd116_2b2n_m14_weight != 0 ) { fdir = (TDirectory *) infile->Get("Cd116_2b2n_m14" ); fdir->cd(); Cd116_2b2n_m14   = (TH1D *) fdir->Get("Cd116_2b2n_m14_h_tot_e_energy" ) -> Clone( "Cd116_2b2n_m14" ) ; Cd116_2b2n_m14 -> Sumw2() ; Cd116_2b2n_m14 -> Scale( Cd116_2b2n_m14_weight / Cd116_2b2n_m14  ->Integral() ) ; bkgdNames.push_back("Cd116_2b2n_m14"); vbkgd.push_back( Cd116_2b2n_m14 ); Cd116_2b2n_m14_i = index; index++; };
	...
	/*  0 */ if( Cd116_Tl208     ) {cfile->createFlatBkgdSystematic( Cd116_Tl208_i    , "Tl208"      , 0.0005 , 0.0005 ,100);};
	/*  1 */ if( Cd116_Ac228     ) {cfile->createFlatBkgdSystematic( Cd116_Ac228_i    , "Tl208"      , 0.0005 , 0.0005 ,100);};
	/*  2 */ if( Cd116_Bi212     ) {cfile->createFlatBkgdSystematic( Cd116_Bi212_i    , "Tl208"      , 0.0005 , 0.0005 ,100);};
	/*  3 */ if( Cd116_Bi214     ) {cfile->createFlatBkgdSystematic( Cd116_Bi214_i    , "Bi214"      , 0.0001 , 0.0001 ,100);};
	/*  4 */ if( Cd116_Pb214     ) {cfile->createFlatBkgdSystematic( Cd116_Pb214_i    , "Bi214"      , 0.0001 , 0.0001 ,100);};
	/*  5 */ if( Mylar_Bi214     ) {cfile->createFlatBkgdSystematic( Mylar_Bi214_i    , "Bi214"      , 0.0001 , 0.0001 ,100);};
	/*  6 */ if( Mylar_Pb214     ) {cfile->createFlatBkgdSystematic( Mylar_Pb214_i    , "Bi214"      , 0.0001 , 0.0001 ,100);};
	/*  7 */ if( Cd116_K40       ) {cfile->createFlatBkgdSystematic( Cd116_K40_i      , "Internal"   , 0.0168 , 0.0074 ,100);};
	/*  8 */ if( Cd116_Pa234m    ) {cfile->createFlatBkgdSystematic( Cd116_Pa234m_i   , "Internal"   , 0.0168 , 0.0074 ,100);};
	/*  9 */ if( SFoil_Bi210     ) {cfile->createFlatBkgdSystematic( SFoil_Bi210_i    , "Internal"   , 0.0168 , 0.0074 ,100);};
	/* 10 */ if( SWire_Bi210     ) {cfile->createFlatBkgdSystematic( SWire_Bi210_i    , "Internal"   , 0.0168 , 0.0074 ,100);};
	/* 11 */ if( SScin_Bi210     ) {cfile->createFlatBkgdSystematic( SScin_Bi210_i    , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 12 */ if( SScin_Bi214     ) {cfile->createFlatBkgdSystematic( SScin_Bi214_i    , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 13 */ if( SScin_Pb214     ) {cfile->createFlatBkgdSystematic( SScin_Pb214_i    , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 14 */ if( SWire_Tl208     ) {cfile->createFlatBkgdSystematic( SWire_Tl208_i    , "Radon"      , 0.0002 , 0.0002 ,100);};
	/* 15 */ if( SWire_Bi214_P1  ) {cfile->createFlatBkgdSystematic( SWire_Bi214_P1_i , "Radon"      , 0.0002 , 0.0002 ,100);};
	/* 16 */ if( SFoil_Bi214     ) {cfile->createFlatBkgdSystematic( SFoil_Bi214_i    , "Radon"      , 0.0002 , 0.0002 ,100);};
	/* 17 */ if( SWire_Pb214     ) {cfile->createFlatBkgdSystematic( SWire_Pb214_i    , "Radon"      , 0.0002 , 0.0002 ,100);};
	/* 18 */ if( SFoil_Pb214     ) {cfile->createFlatBkgdSystematic( SFoil_Pb214_i    , "Radon"      , 0.0002 , 0.0002 ,100);};
	/* 19 */ if( FeShield_Bi214  ) {cfile->createFlatBkgdSystematic( FeShield_Bi214_i , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 20 */ if( FeShield_Tl208  ) {cfile->createFlatBkgdSystematic( FeShield_Tl208_i , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 21 */ if( FeShield_Ac228  ) {cfile->createFlatBkgdSystematic( FeShield_Ac228_i , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 22 */ if( CuTower_Co60    ) {cfile->createFlatBkgdSystematic( CuTower_Co60_i   , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 23 */ if( Air_Bi214_P1    ) {cfile->createFlatBkgdSystematic( Air_Bi214_P1_i   , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 24 */ if( Air_Tl208_P1    ) {cfile->createFlatBkgdSystematic( Air_Tl208_P1_i   , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 25 */ if( PMT_Bi214       ) {cfile->createFlatBkgdSystematic( PMT_Bi214_i      , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 26 */ if( PMT_Tl208       ) {cfile->createFlatBkgdSystematic( PMT_Tl208_i      , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 27 */ if( PMT_Ac228       ) {cfile->createFlatBkgdSystematic( PMT_Ac228_i      , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 28 */ if( PMT_K40         ) {cfile->createFlatBkgdSystematic( PMT_K40_i        , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 29 */ if( ScintInn_K40    ) {cfile->createFlatBkgdSystematic( ScintInn_K40_i   , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 30 */ if( ScintOut_K40    ) {cfile->createFlatBkgdSystematic( ScintOut_K40_i   , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 31 */ if( ScintPet_K40    ) {cfile->createFlatBkgdSystematic( ScintPet_K40_i   , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 32 */ if( MuMetal_Pa234m  ) {cfile->createFlatBkgdSystematic( MuMetal_Pa234m_i , "Externals"  , 0.0130 , 0.0060 ,100);};
	/* 33 */ if( Cd116_2b2n_m14  ) {cfile->createFlatBkgdSystematic( Cd116_2b2n_m14_i , "bb2nu"      , 0.0180 , 0.0180 ,100);};
